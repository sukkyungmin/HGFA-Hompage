...
namespace HangilFA
{
    public static class DbInitializer
    {
		public static void Initialize(IApplicationBuilder app)
		{
            ...
            var context = serviceScope.ServiceProvider.GetService<HangilFADBContext>();
            context.Database.EnsureCreated();

            var _userManager = serviceScope.ServiceProvider.GetRequiredService<UserManager<HangilFAUser>>();
            ...
            if (!context.Users.Any(usr => usr.UserName == "Admin"))
            {
                ...
                var userResult = _userManager.CreateAsync(user, "hgfa7180**").Result;
            }
            ...
            var fliemasters = GetFileMaster();
            foreach (var item in fliemasters)
            {
                if (!context.FileMaster.Any(n => n.Name == item.Name))
                {
                    context.FileMaster.Add(item);
                }
            }
		...
		private static List<NavigationMenu> GetPermissions()
		{
			return new List<NavigationMenu>()
			{
                ...
                new NavigationMenu()
                {
                    Id = new Guid("EEA40D1B-05D2-494C-89BE-41009BDA10ED"),
                    Name = "Create Notices",
                    ControllerName = "Admin",
                    ActionName = "CreateNotices",
                    ParentMenuId = new Guid("13e2f21a-4283-4ff8-bb7a-096e7b89e0f0"),
                    DisplayOrder=3,
                    Visible = true,
                },
		...
		private static List<FileMaster> GetFileMaster()
		{
            return new List<FileMaster>()
            {
                new FileMaster()
                {
                    Name = "PLC",
